...
<div class="form-container">
    <h2>Book Edit (<?php echo $book_id; ?>)</h2>
    <hr>
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $book_id; ?>" method="POST">
        <label for="title">Title</label>
        <input type="text" id="title" name="title" value="<?php echo isset($title) ? $title : $current_book['title']; ?>" maxlength="250">
        <?php if ($title_error): ?>
            <span class="error"><?php echo $title_error; ?></span>
        <?php endif; ?>

        <label for="year">Year</label>
        <input type="text" id="year" name="year" value="<?php echo isset($year) ? $year : $current_book['year']; ?>">
        <?php if ($year_error): ?>
